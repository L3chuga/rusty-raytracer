...
const GROUND_MATERIAL : Material = Material::Lambertian { albedo: Color::new(Vec3::new(0.5,0.5,0.5)) };
const CENTER_MATERIAL : Material = Material::Lambertian { albedo: Color::new(Vec3::new(0.1,0.2,0.5)) };
const LEFT_MATERIAL : Material = Material::Dielectric { refraction_index: 1.5 };
const BUBBLE_MATERIAL : Material = Material::Dielectric { refraction_index: 1.0/1.5 };
const RIGHT_MATERIAL : Material = Material::Metal { albedo: Color::new(Vec3::new(0.8,0.6,0.2)), fuzz: 0.0 };



const S1 : Sphere = Sphere::new(Vec3::new(0.0,1.0,0.0),1.0,LEFT_MATERIAL);
const S2 : Sphere = Sphere::new(Vec3::new(-4.0,1.0,0.0),1.0, Material::Lambertian { albedo: Color::new(Vec3::new(0.4,0.2,0.1)) });
const S3 : Sphere = Sphere::new(Vec3::new(4.0,1.0,0.0),1.0, Material::Metal { albedo : Color::new(Vec3::new(0.7,0.6,0.5)), fuzz : 0.0});
        ...
        for b in -8..8 {
            ...
            if (center-Vec3::new(4.0,0.2,-1.0)).norm_squared() > 0.9 {
                if choose_material < 0.8 {
                    let albedo = Color::new(Vec3::random_vec() * Vec3::random_vec());
                    world.add(Box::new(Sphere::new(center, 0.2, Material::Lambertian { albedo })));
                } else if choose_material < 0.95 {
                    let albedo = Color::new((Vec3::random_vec()+1.0)/2.0);
